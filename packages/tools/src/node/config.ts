import { extname, join, resolve, dirname, isAbsolute } from 'path'
...
import type { BuildOptions as BuildOptions$0 } from './build'
import chalk from 'chalk'
import { pathToFileURL } from 'url'
import { Plugin, resolvePlugins } from './plugins'
...
export interface UserConfig {
   ...
   * deps or some other cache files that generated by edo, which can improve
   * the performance. You can use `--force` flag or manually delete the directory
   * to regenerate the cache files. The value can be either an absolute file
   * system path or a path relative to <root>.
   * Default to `.edo` when no `package.json` is detected.
   * @default 'node_modules/.edo'
   */
  cacheDir?: string
  /**
...
export type SupportCommand = 'commit' | 'build' | 'init' | 'pkg' | 'work'
...
export async function resolveConfig(
  ...
  const [workerPrePlugins, workerNormalPlugins, workerPostPlugins] = sortUserPlugins(config.worker?.plugins as Plugin[])
  ...
  await Promise.all(resolved.worker.plugins.map((p: Plugin) => p.configResolved?.(workerResolved)))
  ;(resolved.plugins as Plugin[]) = await resolvePlugins(resolved, prePlugins, normalPlugins, postPlugins)
  await Promise.all(userPlugins.map((p) => p.configResolved?.(resolved)))

  logger.debug(`using resolved config: %O`, { ...resolved, plugins: resolved.plugins?.map((p) => (p as Plugin).name) })
  ...
  try {
    ...
    if (isESM) {
      ...
      if (isTS) {
        ...
        logger.info(`TS + native esm config loaded in ${getTime()}`, resolvedPath)
      } else {
        userConfig = (await dynamicImport(`${fileUrl}.js?t=${Date.now()}`)).default
        logger.info(`native esm config loaded in ${getTime()}`, resolvedPath)
      }
    }
